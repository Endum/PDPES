...
enum List[A]:
  ...
  def find(predicate: A => Boolean): Int =
    this.zipRight.filter((a, _) => predicate(a)).get(0) match
      case Some(h) => h._2
      case _ => this.length

  def span(pred: A => Boolean): (List[A], List[A]) =
    val part = this.zipRight.partition((_,pos) => pos < find(a => !pred(a)))
    (part._1.map((a, _) => a), part._2.map((a, _) => a))
  ...
  def takeRight(n: Int): List[A] = this.zipRight match
    case (h, p) :: t if p == this.length-n-1 => t.map((a, _) => a)
    case _ :: t => t.map((a, _) => a).takeRight(n)
    case _ => Nil()
